...
static void sync_trig_activate(struct led_classdev *led_cdev);
static void sync_trig_deactivate(struct led_classdev *led_cdev);
static void trig_timer_tick(unsigned long data);
...
static DEFINE_TIMER(trig_timer, trig_timer_tick, 0, 0);
...
static void trig_timer_tick(unsigned long data)
{
	...
	if (atomic_read(&led_count))
		mod_timer(&trig_timer, jiffies + msecs_to_jiffies(DELAY_MS));
}
