...
static int tla2024_find_chan_idx(u32 ainp_in, u32 ainn_in)
{
	int i;
				   ...
				   struct iio_chan_spec const *chan, int *val)
{
	...
	chan_id = tla2024_find_chan_idx(ainp, ainn);
	if (chan_id < 0)
		return chan_id;
...
static int tla2024_init_chan(struct tla2024 *priv, u16 chan_idx,
			     struct iio_chan_spec *chan)
{
	const struct tla2024_channel *tc = &tla2024_all_channels[chan_idx];
	...
	/* if already used */
	if (priv->used_mux_channels & BIT(chan_idx))
		return -EINVAL;

	priv->used_mux_channels |= BIT(chan_idx);

	chan->type = IIO_VOLTAGE;
	chan->channel = tc->ainp;
	chan->channel2 = tc->ainn;
	chan->differential = tc->differential;
	chan->datasheet_name = tc->datasheet_name;
	chan->indexed = 1;
...
static int tla2024_of_init_chan(struct tla2024 *priv, struct device_node *ch,
				struct iio_chan_spec *chan)
{
	u32 ainp, ainn;
	int chan_idx;
	...
	if (ret) {
		ret = of_property_read_u32_index(ch, "diff-channels", 0, &ainp);
		if (ret)
			return ret;

		ret = of_property_read_u32_index(ch, "diff-channels", 1, &ainn);
		if (ret)
			return ret;
	...
	chan_idx = tla2024_find_chan_idx(ainp, ainn);
	if (chan_idx < 0)
		return chan_idx;

	return tla2024_init_chan(priv, chan_idx, chan);
}

static int tla2024_of_init(struct iio_dev *idev)
{
	struct tla2024 *priv = iio_priv(idev);
	...
	channels = devm_kcalloc(&idev->dev, num_channels, sizeof(*channels),
				GFP_KERNEL);
	if (!channels)
		return -ENOMEM;
	...
	for_each_available_child_of_node(node, child) {
		ret = tla2024_of_init_chan(priv, child, &channels[i]);
		if (ret) {
			of_node_put(child);
			return ret;
		}
...
static int tla2024_init(struct iio_dev *idev)
{
	struct tla2024 *priv = iio_priv(idev);
	struct iio_chan_spec *channels;
	int ret, i, num_channels, ainn;
	int chan_idx;

	if (idev->dev.of_node)
		return tla2024_of_init(idev);

	if (priv->model->mux_available) {
		num_channels = 4;
		ainn = -1;
	} else {
		num_channels = 1;
	...
	for (i = 0; i < num_channels; i++) {
		chan_idx = tla2024_find_chan_idx(i, -1);
		if (chan_idx < 0)
			return chan_idx;

		ret = tla2024_init_chan(priv, chan_idx, &channels[i]);
		if (ret)
			return ret;
	}
