			       ...
			       const struct usb_device_id *id)
{
	...
	for (i = 0; i < max_endpoints; ++i) {
		...
		if (num_ports > 1) {
			port->dev.of_node =
				usb_of_get_child_node(interface->dev.of_node,
							i);
			of_node_get(port->dev.of_node);
		} else {
			port->dev.of_node =
				of_node_get(interface->dev.of_node);
		}
