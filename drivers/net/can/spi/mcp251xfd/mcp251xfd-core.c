...
static ssize_t mcp251x_get_spi_crc_retries(struct device *dev,
...
static ssize_t mcp251x_get_spi_crc_errors(struct device *dev,
							struct device_attribute *attr, char *buf)
{
	struct net_device *ndev = to_net_dev(dev);
	struct mcp251xfd_priv *priv = netdev_priv(ndev);
...
DEVICE_ATTR(spi_crc_retries, 0444, mcp251x_get_spi_crc_retries, NULL);
DEVICE_ATTR(spi_crc_errors, 0444, mcp251x_get_spi_crc_errors, NULL);
DEVICE_ATTR(failed, 0444, mcp251x_get_failed, NULL);

static struct attribute *mcp251x_attrs[] = {
	&dev_attr_spi_crc_retries.attr,
	&dev_attr_spi_crc_errors.attr,
